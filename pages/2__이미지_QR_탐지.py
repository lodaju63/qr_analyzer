...
def main():
    ...
    # 업로드된 파일이 있으면 임시 파일로 저장하여 세션 상태 유지
    if uploaded_file is not None:
        # 새 파일이 업로드되었거나 기존 파일과 다른 경우만 저장
        if (st.session_state.uploaded_image_path is None or 
            ...
            st.session_state.uploaded_image_path = temp_file.name
            st.session_state.uploaded_image_name = uploaded_file.name
            # 새 파일 업로드 시 결과 초기화 (선택적)
            # st.session_state.results = None
    
    # 세션 상태에 저장된 이미지 파일이 있으면 사용
    image = None
    if st.session_state.uploaded_image_path and os.path.exists(st.session_state.uploaded_image_path):
        # 이미지 읽기
        image = cv2.imread(st.session_state.uploaded_image_path)
    
    # 업로드된 이미지가 있는 경우 처리
    if image is not None:
        # 업로드된 파일 정보 표시
        if st.session_state.uploaded_image_name:
            st.info(f"📁 현재 이미지: **{st.session_state.uploaded_image_name}**")
        ...
        if st.session_state.results:
            ...
            with download_col1:
                ...
                if results['original_image_visualized'] is not None:
                    ...
                    file_name = st.session_state.uploaded_image_name or "image"
                    st.download_button(
                        label="⬇️ 원본 (시각화)",
                        data=buf.getvalue(),
                        file_name=f"original_visualized_{file_name}",
                        mime="image/jpeg",
                        width='stretch',
            ...
            with download_col2:
                ...
                if results['preprocessed_image_visualized'] is not None:
                    ...
                    st.download_button(
                        label="⬇️ 전처리 (시각화)",
                        data=buf.getvalue(),
                        file_name=f"preprocessed_visualized_{file_name}",
                        mime="image/jpeg",
                        width='stretch',
            ...
            with download_col3:
                ...
                if results['preprocessed_image'] is not None:
                    ...
                    st.download_button(
                        label="⬇️ 전처리 (순수)",
                        data=buf.getvalue(),
                        file_name=f"preprocessed_clean_{file_name}",
                        mime="image/jpeg",
                        width='stretch',
            ...
            if (results['original_image_visualized'] is not None or 
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    if results['original_image_visualized'] is not None:
                        ...
                        buf.seek(0)
                        file_name = st.session_state.uploaded_image_name or "image"
                        zip_file.writestr(f"original_visualized_{file_name}", buf.getvalue())
                    ...
                    if results['preprocessed_image_visualized'] is not None:
                        ...
                        buf.seek(0)
                        file_name = st.session_state.uploaded_image_name or "image"
                        zip_file.writestr(f"preprocessed_visualized_{file_name}", buf.getvalue())
                    ...
                    if results['preprocessed_image'] is not None:
                        ...
                        buf.seek(0)
                        file_name = st.session_state.uploaded_image_name or "image"
                        zip_file.writestr(f"preprocessed_clean_{file_name}", buf.getvalue())
                
                zip_buffer.seek(0)
                file_name = st.session_state.uploaded_image_name or "image"
                zip_filename = f"qr_results_{os.path.splitext(file_name)[0]}.zip"
                st.download_button(
